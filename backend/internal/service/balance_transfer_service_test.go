...
func (s *transferUserRepoStub) GetUsersByIDs(_ context.Context, userIDs []int64) (map[int64]*User, error) {
	s.batchProfileIDs = append([]int64(nil), userIDs...)
...
func TestGetAllTransfers_BatchesSenderAndReceiverProfiles(t *testing.T) {
	repo := &transferRepoStub{
		listAllTotal: 2,
		listAllRecords: []*BalanceTransferRecord{
	...
	userRepo := &transferUserRepoStub{
		users: map[int64]*User{
			1: {ID: 1, Email: "sender@example.com", Username: "sender"},
			2: {ID: 2, Email: "first@example.com", Username: "first"},
			3: {ID: 3, Email: "second@example.com", Username: "second"},
		},
	}
	svc := newTransferTestService(t, repo, userRepo, nil)
