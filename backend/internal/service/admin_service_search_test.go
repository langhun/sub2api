...
type accountRepoStubForAdminList struct {
	...
	listWithFiltersTier     string
	listWithFiltersAccounts []Account
	listWithFiltersResult   *pagination.PaginationResult
	listWithFiltersErr      error
}

func (s *accountRepoStubForAdminList) ListWithFilters(_ context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, tier string) ([]Account, *pagination.PaginationResult, error) {
...
func TestAdminService_ListAccounts_WithTier(t *testing.T) {
	t.Run("tier 参数正常传递到 repository 层", func(t *testing.T) {
		repo := &accountRepoStubForAdminList{
		...
		accounts, total, err := svc.ListAccounts(context.Background(), 1, 20, PlatformOpenAI, AccountTypeOAuth, StatusActive, "acc3", 0, PrivacyModeCFBlocked, "openai:plus", "", "")
		require.NoError(t, err)
		require.Equal(t, int64(1), total)
		require.Equal(t, []Account{{ID: 3, Name: "acc3"}}, accounts)
		require.Equal(t, "openai:plus", repo.listWithFiltersTier)
	})
}
