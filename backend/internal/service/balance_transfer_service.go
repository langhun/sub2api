...
const (
	...
	userSearchLimit         = 10
	redPacketMoneyScale     = 8
)

var redPacketMinUnit = decimal.RequireFromString("0.01")
...
type balanceTransferUserProfileBatchRepository interface {
...
func (s *BalanceTransferService) GetAllTransfers(ctx context.Context, filter *TransferFilter, page, pageSize int) ([]*BalanceTransferRecord, int, error) {
	...
	usernames, err := s.getTransferUsernames(ctx, userIDs)
	if err != nil {
		return nil, 0, err
	}
	for _, r := range records {
		r.SenderEmail = emails[r.SenderID]
		r.SenderUsername = usernames[r.SenderID]
		r.ReceiverEmail = emails[r.ReceiverID]
		r.ReceiverUsername = usernames[r.ReceiverID]
	}
...
func (s *BalanceTransferService) getTransferUsernames(ctx context.Context, userIDs []int64) (map[int64]string, error) {
	usernames := make(map[int64]string, len(userIDs))
	if len(userIDs) == 0 {
		return usernames, nil
	}
	if repo, ok := s.userRepo.(balanceTransferUserProfileBatchRepository); ok {
		batched, err := repo.GetUsersByIDs(ctx, userIDs)
		if err != nil {
			return nil, fmt.Errorf("batch get transfer users: %w", err)
		}
		for id, user := range batched {
			if user != nil {
				usernames[id] = strings.TrimSpace(user.Username)
			}
		}
		return usernames, nil
	}
	for _, uid := range userIDs {
		u, err := s.userRepo.GetByID(ctx, uid)
		if err == nil && u != nil {
			usernames[uid] = strings.TrimSpace(u.Username)
		}
	}
	return usernames, nil
}
...
func (s *BalanceTransferService) CreateRedPacket(ctx context.Context, senderID int64, totalAmount float64, count int, redPacketType string, memo *string) (*RedPacketRecord, error) {
	...
	if totalAmount < minRequired {
		return nil, infraerrors.BadRequest(
			"REDPACKET_AMOUNT_TOO_SMALL",
			fmt.Sprintf("minimum amount for %d packets is %.2f", count, minRequired),
		).WithMetadata(map[string]string{
			"count":  fmt.Sprintf("%d", count),
			"amount": decimal.NewFromFloat(minRequired).StringFixed(2),
		})
	}
	feeRate := cfg.FeeRate
...
func (s *BalanceTransferService) calculateClaimAmount(rp *RedPacketRecord) float64 {
	...
	remaining := redPacketAmountDecimal(rp.RemainingAmount)
	if !remaining.GreaterThan(decimal.Zero) {
		return 0
	}
	if rp.RedPacketType == "equal" {
		return remaining.Div(decimal.NewFromInt(int64(rp.RemainingCount))).Round(redPacketMoneyScale).InexactFloat64()
	}
	if rp.RemainingCount == 1 {
		return remaining.InexactFloat64()
	}
	maxAllowed := remaining.Sub(redPacketMinUnit.Mul(decimal.NewFromInt(int64(rp.RemainingCount - 1)))).Round(redPacketMoneyScale)
	if !maxAllowed.GreaterThan(decimal.Zero) {
		if remaining.GreaterThan(redPacketMinUnit) {
			return redPacketMinUnit.InexactFloat64()
		}
		return remaining.InexactFloat64()
	}
	upperBoundCents := maxAllowed.
		Div(decimal.NewFromInt(int64(rp.RemainingCount))).
		Mul(decimal.NewFromInt(2)).
		Mul(decimal.NewFromInt(100)).
		Floor().
		IntPart()
	if upperBoundCents <= 0 {
		if maxAllowed.GreaterThan(redPacketMinUnit) {
			return redPacketMinUnit.InexactFloat64()
		}
		return maxAllowed.InexactFloat64()
	}
	n, err := rand.Int(rand.Reader, big.NewInt(upperBoundCents))
	if err != nil {
		return maxAllowed.Div(decimal.NewFromInt(int64(rp.RemainingCount))).Round(redPacketMoneyScale).InexactFloat64()
	}
	amount := decimal.NewFromInt(n.Int64()).Div(decimal.NewFromInt(100)).Add(redPacketMinUnit)
	if amount.LessThan(redPacketMinUnit) {
		amount = redPacketMinUnit
	}
	if amount.GreaterThan(maxAllowed) {
		amount = maxAllowed
	}
	return amount.Round(redPacketMoneyScale).InexactFloat64()
}

func redPacketAmountDecimal(value float64) decimal.Decimal {
	return decimal.NewFromFloat(value).Round(redPacketMoneyScale)
}
