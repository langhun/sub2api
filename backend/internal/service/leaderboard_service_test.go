...
func newLeaderboardSQLMockClient(t *testing.T) (*dbent.Client, *sql.DB, sqlmock.Sqlmock, func()) {
	t.Helper()
	...
	return client, db, mock, cleanup
}

func TestLeaderboardService_GetBalanceLeaderboard_UsesUsersBalanceEvenWhenBankTableExists(t *testing.T) {
	client, db, mock, cleanup := newLeaderboardSQLMockClient(t)
	t.Cleanup(cleanup)

	countQuery := `SELECT COUNT\("users"\."id"\) FROM "users"`
	dataQuery := `SELECT .* FROM "users".*ORDER BY "users"\."balance" DESC.*LIMIT 10`
	...
	mock.ExpectQuery(dataQuery).
		WillReturnRows(
			sqlmock.NewRows([]string{
				"id", "created_at", "updated_at", "deleted_at", "email", "password_hash", "role",
				"balance", "concurrency", "status", "username", "notes", "totp_secret_encrypted",
				"totp_enabled", "totp_enabled_at", "signup_source", "last_login_at", "last_active_at",
				"balance_notify_enabled", "balance_notify_threshold_type", "balance_notify_threshold",
				"balance_notify_extra_emails", "total_recharged", "rpm_limit",
			}).
				AddRow(
					int64(2), time.Now(), time.Now(), nil, "alpha@example.com", "hash", "user",
					164.0, 1, "active", "Alpha", "", nil,
					false, nil, "email", nil, nil,
					false, "percentage", nil, "", 0.0, 0,
				).
				AddRow(
					int64(3), time.Now(), time.Now(), nil, "beta@example.com", "hash", "user",
					85.9, 1, "active", "", "", nil,
					false, nil, "email", nil, nil,
					false, "percentage", nil, "", 0.0, 0,
				),
	...
	svc := &LeaderboardService{entClient: client, db: db}
	result, err := svc.GetBalanceLeaderboard(context.Background(), 1, 10, false)
	require.NoError(t, err)
	require.Equal(t, int64(2), result.Total)
	require.Len(t, result.Entries, 2)
	require.Equal(t, "Alpha", result.Entries[0].Username)
	require.Equal(t, 164.0, result.Entries[0].Value)
	require.Equal(t, 33, result.Entries[0].ExtraInt)
	require.Equal(t, "bet***", result.Entries[1].Username)
	require.Equal(t, 85.9, result.Entries[1].Value)
	require.Equal(t, 11, result.Entries[1].ExtraInt)
