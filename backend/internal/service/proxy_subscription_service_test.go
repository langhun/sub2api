...
type proxySubscriptionSidecarStub struct {
	lastRequest ProxySubscriptionSidecarUpsertRequest
}

func (s *proxySubscriptionSidecarStub) HealthCheck(ctx context.Context) error { return nil }
func (s *proxySubscriptionSidecarStub) UpsertRuntime(_ context.Context, req ProxySubscriptionSidecarUpsertRequest) (*ProxySubscriptionSidecarUpsertResponse, error) {
	s.lastRequest = req
	return &ProxySubscriptionSidecarUpsertResponse{
...
func (s *proxySubscriptionSidecarStub) DeleteRuntime(ctx context.Context, runtimeID string) error {
	return nil
}
func (s *proxySubscriptionSidecarStub) CheckRuntime(ctx context.Context, runtimeID string) error {
	return nil
}
...
func TestProxySubscriptionService_RefreshSource_MaterializesDirectProxy(t *testing.T) {
	...
	svc := NewProxySubscriptionService(sourceRepo, nodeRepo, proxyRepo, nil, nil, &config.Config{
...
func TestProxySubscriptionService_RefreshSource_MaterializesSidecarProxy(t *testing.T) {
	sourceRepo := &proxySubscriptionSourceRepoStub{
		sources: map[int64]*ProxySubscriptionSource{
			2: {
				ID:                   2,
	...
	nodeRepo := &proxySubscriptionNodeRepoStub{}
	proxyRepo := &proxyRepoSubscriptionStub{}
	sidecar := &proxySubscriptionSidecarStub{}
	...
	svc := NewProxySubscriptionService(sourceRepo, nodeRepo, proxyRepo, nil, sidecar, &config.Config{
		ProxySubscriptions: config.ProxySubscriptionsConfig{DefaultRefreshIntervalHours: 6},
	})
	...
	if sidecar.lastRequest.NodeType != ProxyNodeTypeVMess {
		t.Fatalf("unexpected sidecar request = %#v", sidecar.lastRequest)
	}
}
...
func TestProxySubscriptionService_RefreshSource_DisablesStaleMaterializedProxy(t *testing.T) {
	...
	proxyRepo := &proxyRepoSubscriptionStub{
		proxies: map[int64]*Proxy{
			21: {
				...
				SubscriptionNodeID:    ptrInt64Sub(11),
			},
		},
		nextID: 30,
	}
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte("http,1.2.3.4,8080,user,pass,new-proxy"))
	}))
	defer server.Close()
	...
	if _, ok := proxyRepo.proxies[21]; ok {
		t.Fatalf("expected stale proxy 21 to be deleted when unbound")
	}
}
...
func TestProxySubscriptionService_RefreshSource_DisablesButKeepsBoundStaleProxy(t *testing.T) {
	...
	proxyRepo := &proxyRepoSubscriptionStub{
		proxies: map[int64]*Proxy{
			41: {
				...
				SubscriptionNodeID:    ptrInt64Sub(31),
			},
		},
		accountCounts: map[int64]int64{
			41: 2,
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte("http,1.1.1.1,8080,user,pass,new-proxy"))
	}))
	defer server.Close()
	...
	if !ok {
		t.Fatal("expected stale bound proxy to remain")
	}
	if item.Status != StatusDisabled {
		t.Fatalf("expected stale bound proxy to be disabled, got %s", item.Status)
	}
}
