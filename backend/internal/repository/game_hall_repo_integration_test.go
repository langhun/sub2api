...
`, gameHallJackpotCode, 100.0)
	...
	require.NoError(t, err)
	require.Equal(t, service.GameSlotsRuleVersion, ruleVersion)
...
`, user.ID).Scan(&dgBalance)
	...
	replayed, err := repo.CommitExchange(ctx, service.GameExchangePlan{
		UserID: user.ID, Direction: service.GameExchangeBalanceToDG, Amount: 20, IdempotencyKey: idempotencyKey,
	})
	require.NoError(t, err)
