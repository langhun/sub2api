...
func (s *HTTPUpstreamSuite) TestAcquireClientWithTLS_OverLimitFallsBackToTransient() {
	...
	entry1, err := svc.getClientEntryWithTLS("", 1, 1, profile, service.HTTPUpstreamProfileOpenAI, true, true)
	require.NoError(s.T(), err, "expected first TLS acquire to succeed")
	require.NotNil(s.T(), entry1, "expected entry")

	entry2, err := svc.getClientEntryWithTLS("http://proxy-b:8080", 2, 1, profile, service.HTTPUpstreamProfileOpenAI, true, true)
