...
func (r *balanceTransferRepo) GetByIDForUpdate(ctx context.Context, id int64) (*service.BalanceTransferRecord, error) {
	client := clientFromContext(ctx, r.client)
	t, err := client.BalanceTransfer.Query().Where(balancetransfer.IDEQ(id)).ForUpdate().Only(ctx)
	if err != nil {
		return nil, fmt.Errorf("get balance transfer %d for update: %w", id, err)
	}
	return toTransferRecord(t), nil
}

func (r *balanceTransferRepo) LockUser(ctx context.Context, userID int64) error {
	client := clientFromContext(ctx, r.client)
...
func (r *balanceTransferRepo) DeductBalanceIfSufficient(ctx context.Context, userID int64, amount float64) (bool, error) {
	client := clientFromContext(ctx, r.client)
