...
const (
	...
	// 仅限制可复用客户端缓存；缓存已满且全部活跃时，请求会降级为瞬时客户端
...
type upstreamClientEntry struct {
	...
	cached       bool         // 是否为缓存客户端；false 表示超限时临时创建的瞬时客户端
...
func newUpstreamClientEntry(client *http.Client, proxyKey, poolKey, protocolMode string, nowUnix int64, markInFlight bool, cached bool) *upstreamClientEntry {
...
func (s *httpUpstreamService) getClientEntryWithTLS(proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile, upstreamProfile service.HTTPUpstreamProfile, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	...
	// 超出缓存上限时尝试淘汰；若所有缓存条目都处于活跃中，则降级为瞬时客户端
	useTransient := false
	...
	if useTransient {
		...
		return newUpstreamClientEntry(client, proxyKey, poolKey, upstreamProtocolModeDefault, nowUnix, markInFlight, false), nil
	}
	...
	entry := newUpstreamClientEntry(client, proxyKey, poolKey, upstreamProtocolModeDefault, nowUnix, markInFlight, true)
...
func (s *httpUpstreamService) getClientEntry(proxyURL string, accountID int64, accountConcurrency int, profile service.HTTPUpstreamProfile, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	...
	if useTransient {
		...
		return newUpstreamClientEntry(client, proxyKey, poolKey, protocolMode, nowUnix, markInFlight, false), nil
	}
	...
	entry := newUpstreamClientEntry(client, proxyKey, poolKey, protocolMode, nowUnix, markInFlight, true)
	s.clients[cacheKey] = entry
...
func (s *httpUpstreamService) finishRequest(entry *upstreamClientEntry) {
	if entry == nil {
		return
	}
	atomic.AddInt64(&entry.inFlight, -1)
