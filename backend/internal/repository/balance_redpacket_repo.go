...
func (r *balanceRedPacketRepo) DecrementClaim(ctx context.Context, id int64, amount float64) (*service.RedPacketRecord, error) {
	client := clientFromContext(ctx, r.client)
	rows, err := client.QueryContext(ctx,
		`UPDATE balance_redpackets
		 ...
		 RETURNING id, sender_id, total_amount, total_count, remaining_amount, remaining_count,
		           redpacket_type, fee, fee_rate, code, status, memo, expire_at, created_at`,
		amount, id,
	)
	if err != nil {
		return nil, err
	...
	if !rows.Next() {
		...
		return nil, fmt.Errorf("red packet exhausted or not available")
	}
	var rp service.RedPacketRecord
	if err := rows.Scan(
		&rp.ID, &rp.SenderID, &rp.TotalAmount, &rp.TotalCount,
		&rp.RemainingAmount, &rp.RemainingCount, &rp.RedPacketType,
		&rp.Fee, &rp.FeeRate, &rp.Code, &rp.Status, &rp.Memo,
		&rp.ExpireAt, &rp.CreatedAt,
	); err != nil {
		return nil, err
	}
	return &rp, rows.Err()
...
func (r *balanceRedPacketRepo) CreateClaim(ctx context.Context, claim *service.RedPacketClaimRecord) error {
	...
	saved, err := client.BalanceRedPacketClaim.Create().
		...
		Save(ctx)
	if err != nil {
		return translatePersistenceError(err, nil, service.ErrRedPacketAlreadyClaimed)
	}
	claim.ID = saved.ID
	return nil
}
...
func (r *balanceRedPacketRepo) ReturnRemaining(ctx context.Context, id int64, senderID int64) (float64, error) {
	client := clientFromContext(ctx, r.client)
	rp, err := client.BalanceRedPacket.Query().Where(balanceredpacket.IDEQ(id)).ForUpdate().Only(ctx)
	if err != nil {
		return 0, err
	}
	// The row lock is shared with claims. A claim that committed first reduces
	// the authoritative remaining amount; an expiry that committed first makes
	// subsequent claims observe the expired state.
	if rp.Status != "active" || !time.Now().After(rp.ExpireAt) {
		return 0, nil
	}
