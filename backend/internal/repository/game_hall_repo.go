...
func (r *gameHallRepository) CommitSlotRound(ctx context.Context, plan service.GameSlotRoundPlan) (*service.GamePlayResult, error) {
	...
	err := r.runInTx(ctx, func(txCtx context.Context) error {
		...
		dgAfter := roundDBAmount(dgBalance - plan.BetAmount + plan.PayoutAmount)
		jackpotAfter := roundDBAmount(jackpotBalance + plan.BetAmount - plan.PayoutAmount)
		if jackpotAfter < 0 {
			jackpotAfter = 0
		}
		...
		result = &service.GamePlayResult{
			...
			PayoutAmount:    roundDBAmount(plan.PayoutAmount),
			NetAmount:       roundDBAmount(plan.NetAmount),
			Multiplier:      plan.Multiplier,
			DGBalanceBefore: roundDBAmount(dgBalance),
			DGBalanceAfter:  dgAfter,
...
`, userID)
	if err != nil {
		return 0, fmt.Errorf("lock user main balance: %w", err)
	}
	defer func() { _ = rows.Close() }()
	...
	if err := rows.Scan(&balance); err != nil {
		return 0, fmt.Errorf("scan user main balance: %w", err)
	}
