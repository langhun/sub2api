<template>
  <div class="space-y-3">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <button
        type="button"
        class="btn btn-secondary gap-2 border-gray-200 bg-gray-100 px-3 text-gray-700 hover:bg-gray-100 dark:border-dark-600 dark:bg-dark-700 dark:text-gray-300 dark:hover:bg-gray-100"
      ...
      <div class="relative" @click.stop>
        ...
        <div
          ...
          class="absolute right-0 z-50 mt-2 w-72 origin-top-right overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800"
        >
          <div class="space-y-3 p-3">
            ...
            <div class="grid gap-2">
              <button
                class="flex items-center gap-3 rounded-xl border border-gray-200 px-3 py-2.5 text-left transition hover:border-[var(--foreground)] hover:bg-[var(--muted)] dark:border-gray-700"
                data-test="proxy-toolbar-import"
                @click="$emit('open-import')"
              >
                <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-[var(--border)] bg-[var(--muted)] text-[var(--info)]"><Icon name="upload" size="sm" /></span>
                <span class="min-w-0 flex-1 text-sm text-gray-700 dark:text-gray-200">{{ t('admin.proxies.dataImport') }}</span>
              </button>

              <button
                class="flex items-center gap-3 rounded-xl border border-gray-200 px-3 py-2.5 text-left transition hover:border-gray-200 hover:bg-gray-100 dark:border-gray-700 dark:hover:border-gray-200 dark:hover:bg-gray-100"
                data-test="proxy-toolbar-export"
                @click="$emit('open-export')"
              >
                <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 text-gray-700 dark:bg-dark-700 dark:text-gray-300"><Icon name="download" size="sm" /></span>
              ...
              <button
                ...
                class="flex items-center gap-3 rounded-xl border border-gray-200 px-3 py-2.5 text-left transition hover:border-gray-300 hover:bg-gray-50/60 dark:border-gray-700 dark:hover:border-gray-700 dark:hover:bg-gray-900/10"
                :title="t('admin.users.columnSettings')"
                data-test="proxy-toolbar-columns"
              >
                <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-gray-50 text-gray-600 dark:bg-gray-900/30 dark:text-gray-300">
      ...
      <div
        ...
        class="inline-flex items-center rounded-full border border-gray-200 bg-gray-100 px-3 py-1.5 text-xs text-gray-700 dark:border-dark-600 dark:bg-dark-700 dark:text-gray-300"
      >
        {{ t('common.selectedCount', { count: selectedCount }) }} · {{ t('admin.proxies.batchBarHint') }}
      </div>

      <button @click="$emit('create-proxy')" class="btn btn-secondary">
