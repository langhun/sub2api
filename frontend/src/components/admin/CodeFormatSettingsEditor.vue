...
const characterSets = computed((): { value: CodeCharacterSet; label: string }[] => [{ value: 'uppercase', label: t('admin.codeFormat.uppercase') }, { value: 'numeric', label: t('admin.codeFormat.numeric') }, { value: 'alphanumeric', label: t('admin.codeFormat.alphanumeric') }, { value: 'hex', label: t('admin.codeFormat.hex') }])
...
function sample(set: CodeCharacterSet) { return { uppercase: 'ABCDEFGH', numeric: '27490163', alphanumeric: 'A7K2P9Q4', hex: 'A4F09C2E' }[set] }
...
function combinationSpace(rule: CodeFormatRule) { const alphabet = { uppercase: 26, numeric: 10, alphanumeric: 36, hex: 16 }[rule.character_set] || 0; const exponent = Math.max(0, rule.group_length * rule.group_count); if (!alphabet || !exponent) return '0'; const logarithm = exponent * Math.log10(alphabet); return logarithm < 15 ? Math.pow(alphabet, exponent).toLocaleString() : `10^${logarithm.toFixed(1)}` }
function ruleError(rule: CodeFormatRule) { if (!['uppercase', 'numeric', 'alphanumeric', 'hex'].includes(rule.character_set)) return t('admin.codeFormat.invalidCharset'); if (!Number.isInteger(rule.group_length) || rule.group_length < 1 || rule.group_length > 32) return t('admin.codeFormat.invalidGroupLength'); if (!Number.isInteger(rule.group_count) || rule.group_count < 1 || rule.group_count > 16) return t('admin.codeFormat.invalidGroupCount'); if (rule.separator.length > 1 || (rule.separator && !/^[\x21-\x7E]$/.test(rule.separator))) return t('admin.codeFormat.invalidSeparator'); if (rule.prefix && (!/^[\x21-\x7E]+$/.test(rule.prefix) || (rule.separator && rule.prefix.includes(rule.separator)))) return t('admin.codeFormat.invalidPrefix'); return '' }
