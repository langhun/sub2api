    ...
    <div v-if="user" class="space-y-4">
      <div class="flex items-center gap-3 rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        <div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary-100 dark:bg-primary-900/30">
          <span class="text-lg font-medium text-primary-700 dark:text-primary-300">{{ getUserDisplayInitial(user) }}</span>
        </div>
        <div><p class="font-medium text-gray-900 dark:text-white">{{ getPreferredUserDisplayName(user, '#' + user.id) }}</p><p v-if="getSecondaryUserEmail(user)" class="text-sm text-gray-500 dark:text-dark-400">{{ getSecondaryUserEmail(user) }}</p></div>
      </div>
