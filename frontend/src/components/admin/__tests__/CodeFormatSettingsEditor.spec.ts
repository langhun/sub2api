...
function mountEditor(modelValue = settings()) {
  return mount(CodeFormatSettingsEditor, {
    props: { modelValue },
    global: { plugins: [createI18n({ legacy: false, locale: 'en', missingWarn: false, fallbackWarn: false, messages: { en: { admin: { codeFormat: { combinationSpace: 'Available combinations: {count}' } } } } })] },
  })
}

describe('CodeFormatSettingsEditor', () => {
  it('renders a live formatted preview and accepts valid rules', () => {
  ...
  it('accepts the backend maximum of 16 groups', () => {
    const modelValue = settings()
    modelValue.balance.group_count = 16
    const wrapper = mountEditor(modelValue)
    expect(wrapper.emitted('validity')?.at(-1)).toEqual([true])
  })

  it('supports a custom single-character separator and shows combination space', async () => {
    const modelValue = settings()
    modelValue.balance.separator = '.'
    const wrapper = mountEditor(modelValue)
    expect(wrapper.text()).toContain('CODE.')
    expect(wrapper.find('input[maxlength="1"]').exists()).toBe(true)
    expect(wrapper.text()).toContain('admin.codeFormat.combinationSpace')
  })
})
