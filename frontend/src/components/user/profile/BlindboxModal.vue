<template>
  <Teleport to="body">
    <Transition name="modal">
      <div
        v-if="show && result"
        class="modal-overlay blindbox-overlay"
        style="z-index: 60"
        ...
        <div class="blindbox-container" @click.stop>
          ...
          <div class="blindbox-card">
            <div class="blindbox-card-inner">
              <div v-if="result.rarity === 'legendary'" class="blindbox-sparkles">
                <span
                  v-for="i in 8"
                  :key="i"
                  class="sparkle"
...
const rewardText = computed(() => {
  ...
  const rewardValue = props.result.reward_value
  switch (props.result.reward_type) {
    case 'balance':
      return t('checkin.blindboxBalanceReward', { value: rewardValue.toFixed(2) })
    case 'concurrency':
      return t('checkin.blindboxConcurrencyReward', { value: rewardValue })
    case 'subscription':
      return t('checkin.blindboxSubscriptionReward', { days: rewardValue })
    case 'invitation_code':
      return t('checkin.blindboxInvitationReward')
    default:
      return `${props.result.reward_type}: ${rewardValue}`
  }
})

const rarityGlowClass = computed(() => (props.result ? `glow-${props.result.rarity}` : ''))
const rarityBgClass = computed(() => (props.result ? `icon-${props.result.rarity}` : ''))
const rarityBadgeClass = computed(() => (props.result ? `badge-${props.result.rarity}` : ''))
const rarityTextClass = computed(() => (props.result ? `text-${props.result.rarity}` : ''))
const rarityBtnClass = computed(() => (props.result ? `btn-${props.result.rarity}` : ''))
