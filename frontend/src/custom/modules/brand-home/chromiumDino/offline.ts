...
import {assert, HIDDEN_CLASS, loadTimeData} from './platform_shims.js';
...
export class Runner implements ImageSpriteProvider, GameStateProvider,
  ...
  private resizeTimerId?: ReturnType<typeof window.setInterval>;
  ...
  private fallbackSounds: Partial<Record<RunnerSounds, HTMLAudioElement>> = {};
  ...
  private readonly boundResize = this.debounceResize.bind(this);
  private readonly boundStartGame = this.startGame.bind(this);
  private readonly boundVisibilityChange = this.onVisibilityChange.bind(this);
  private readonly boundDarkModeChange = (e: MediaQueryListEvent) => {
  ...
  /**
   * The Chromium error page owns this singleton for its entire document
   * lifetime. This embedding is a SPA, so release document-level listeners
   * before the route can mount a new Runner.
   */
  ...
  /** Removes resources that Chromium normally keeps for the page lifetime. */
  dispose() {
    this.stop();
    ...
    document.removeEventListener(RunnerEvents.VISIBILITY, this.boundVisibilityChange);
    window.removeEventListener(RunnerEvents.RESIZE, this.boundResize);
    window.removeEventListener(RunnerEvents.BLUR, this.boundVisibilityChange);
    window.removeEventListener(RunnerEvents.FOCUS, this.boundVisibilityChange);
    window.removeEventListener(RunnerEvents.GAMEPADCONNECTED, this);
    this.darkModeMediaQuery?.removeListener(this.boundDarkModeChange);
    this.containerEl?.remove();
    this.slowSpeedCheckboxLabel?.remove();
    this.touchController?.remove();
    document.querySelector<HTMLElement>('.' + RunnerClasses.ICON)?.style.removeProperty('visibility');
    document.body.classList.remove(RunnerClasses.ARCADE_MODE);
    document.documentElement.classList.remove(RunnerClasses.INVERTED);
  ...
  /**
   * Chromium normally receives these sounds as inline base64. This embed
   * serves the same original files from the public asset directory.
   */
  private loadSounds() {
    if (IS_IOS) {
      return;
    ...
    const soundLoads: Array<Promise<void>> = [];
    for (const sound in RunnerSounds) {
      ...
      assert(audioElement);
      const soundId = sound as keyof typeof RunnerSounds;
      const runnerSound = RunnerSounds[soundId];
      const fallbackAudio = new Audio(audioElement.src);
      fallbackAudio.preload = 'auto';
      this.fallbackSounds = {
        ...this.fallbackSounds,
        [runnerSound]: fallbackAudio,
      };
      soundLoads.push(fetch(audioElement.src)
  ...
  private init() {
    ...
    this.startListening();
    // Decode while the idle animation is visible so the first jump has its
    // original button sound ready. resume() is repeated from the input path.
    this.loadSounds();
    this.update();

    window.addEventListener(RunnerEvents.RESIZE, this.boundResize);
    ...
    darkModeMediaQuery.addListener(this.boundDarkModeChange);
  }
  ...
  private playIntro() {
    if (!this.activated && !this.crashed) {
      ...
      this.containerEl.addEventListener(RunnerEvents.ANIM_END, this.boundStartGame);
  ...
  private startGame() {
    ...
    document.addEventListener(RunnerEvents.VISIBILITY, this.boundVisibilityChange);

    window.addEventListener(RunnerEvents.BLUR, this.boundVisibilityChange);

    window.addEventListener(RunnerEvents.FOCUS, this.boundVisibilityChange);
  }
  ...
  private update() {
    ...
    if (this.playing) {
      ...
      if (!this.hasAudioCuesInternal && playAchievementSound) {
        this.playSound(this.soundFx.SCORE, RunnerSounds.SCORE);
      }
        ...
        if ((e instanceof KeyboardEvent &&
          ...
          if (!this.tRex.jumping && !this.tRex.ducking) {
            if (this.hasAudioCuesInternal) {
              ...
              this.playSound(this.soundFx.BUTTON_PRESS, RunnerSounds.BUTTON_PRESS);
            }
  ...
  private gameOver() {
    ...
    assert(this.tRex);
    assert(this.containerEl);
    this.playSound(this.soundFx.HIT, RunnerSounds.HIT);
    vibrate(200);
   ...
   * Play a sound.
   */
  private playSound(soundBuffer?: AudioBuffer, fallbackSound?: RunnerSounds) {
    if (soundBuffer) {
      ...
      const audioElement = this.fallbackSounds[fallbackSound];
      if (audioElement) {
