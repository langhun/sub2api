...
import {
  ...
  type DirectTransferReceiver,
  type DirectTransferRecord,
  type DirectTransferStats,
  type DirectTransferValidation,
} from '../api'
...
const stats = ref<DirectTransferStats>({ total_sent: 0, total_received: 0, total_fee_paid: 0 })
const receiverQuery = ref('')
const receiver = ref<DirectTransferReceiver | null>(null)
const receiverCandidates = ref<DirectTransferReceiver[]>([])
...
const preview = ref<DirectTransferValidation | null>(null)
const formError = ref('')
const submitting = ref(false)
const confirming = ref(false)
const history = ref<DirectTransferRecord[]>([])
...
async function searchReceivers() {
  ...
  formError.value = ''
  try {
    const result = await searchDirectTransferReceivers(query)
...
function selectReceiver(candidate: DirectTransferReceiver) {
...
async function calculatePreview() {
  ...
  try {
    preview.value = await validateDirectTransfer(receiver.value.receiver_id, form.amount)
    return true
  } catch (error) {
    preview.value = null
...
async function loadStats() {
  try {
    stats.value = await getDirectTransferStats()
  } catch {
...
async function loadHistory() {
  ...
  historyError.value = ''
  try {
    const result = await getDirectTransferHistory({ role: historyRole.value, page: page.value, page_size: pageSize })
...
async function submitTransfer() {
  ...
  if (transferAttempt.value?.signature !== signature) {
    transferAttempt.value = { signature, key: createDirectTransferIdempotencyKey('balance-transfer') }
  }
  try {
    const result = await submitDirectTransfer(receiver.value.receiver_id, form.amount, form.memo || undefined, transferAttempt.value.key)
