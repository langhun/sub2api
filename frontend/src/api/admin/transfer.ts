...
const adminTransferAPI = {
  ...
  getFeeStats(params: { start_time?: string; end_time?: string }) {
    return apiClient.get<DailyFeeStat[]>('/admin/transfers/stats', { params }).then(res => res.data)
  },

  listRedPackets(params: { page?: number; page_size?: number }) {
    return apiClient.get<{ items: any[]; total: number; page: number; page_size: number }>('/admin/redpackets', { params }).then(res => res.data)
  },
}
