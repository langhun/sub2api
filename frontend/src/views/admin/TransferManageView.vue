        ...
        >
          <template #cell-sender_id="{ value, row }">
            <div class="flex flex-col">
              <span class="text-sm text-gray-900 dark:text-white">{{ getPreferredUserDisplayName({ username: row.sender_username, email: row.sender_email }, '#' + value) }}</span>
              <span v-if="getSecondaryUserEmail({ username: row.sender_username, email: row.sender_email })" class="text-xs text-gray-400 dark:text-dark-500">
                {{ getSecondaryUserEmail({ username: row.sender_username, email: row.sender_email }) }}
              </span>
          ...
          <template #cell-receiver_id="{ value, row }">
            <div class="flex flex-col">
              <span class="text-sm text-gray-900 dark:text-white">{{ getPreferredUserDisplayName({ username: row.receiver_username, email: row.receiver_email }, '#' + value) }}</span>
              <span v-if="getSecondaryUserEmail({ username: row.receiver_username, email: row.receiver_email })" class="text-xs text-gray-400 dark:text-dark-500">
                {{ getSecondaryUserEmail({ username: row.receiver_username, email: row.receiver_email }) }}
              </span>
              <span class="text-xs text-gray-400 dark:text-dark-500">#{{ value }}</span>
            </div>
          </template>
            ...
            <div v-for="(target, i) in batchTargets" :key="i" class="rounded-lg border border-gray-200 p-3 dark:border-dark-700">
              <div class="flex items-center gap-2">
                  ...
                  <div v-if="target.results && target.results.length > 0 && !target.selected" class="absolute left-0 right-0 top-full z-10 mt-1 max-h-36 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg dark:border-dark-600 dark:bg-dark-800">
                    <button v-for="u in target.results" :key="u.id" type="button" @click="selectBatchUser(i, u)"
                      ...
                      <div class="flex-1 overflow-hidden">
                        <span class="block truncate text-gray-900 dark:text-white">{{ getPreferredUserDisplayName(u, '#' + u.id) }}</span>
                        <span v-if="getSecondaryUserEmail(u)" class="block truncate text-xs text-gray-400">{{ getSecondaryUserEmail(u) }}</span>
                      </div>
                ...
                <div v-if="target.selected" class="flex items-center gap-1">
                  <span class="rounded-md bg-primary-50 px-2 py-1 text-xs font-medium text-primary-700 dark:bg-primary-900/20 dark:text-primary-300">{{ getPreferredUserDisplayName(target.selected, '#' + target.selected.id) }}</span>
...
function selectBatchUser(i: number, u: SimpleUser) {
  const target = batchTargets[i]
  target.selected = u
  target.query = getPreferredUserDisplayName(u, u.email)
  target.results = []
}
