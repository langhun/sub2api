...
describe('admin ProxiesView pool state', () => {
  ...
  it('opens the subscription source dialog and loads sources', async () => {
    const wrapper = mount(ProxiesView, {
      global: {
        stubs: {
          AppLayout: { template: '<div><slot /></div>' },
  ...
  it('loads every subscription page when the subscription source dialog is opened', async () => {
    listProxySubscriptions
  ...
  it('keeps the refresh shortcut bound to the proxy list outside the subscription source dialog', async () => {
    mountProxiesView()
    await flushPromises()
