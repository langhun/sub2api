  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">{{ t('admin.blindbox.title') }}</h1>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.blindbox.description') }}</p>
        </div>
      ...
      <div v-if="stats" class="grid grid-cols-3 gap-4">
        ...
        <div class="card p-4 text-center">
          <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ stats.enabled_items }}</p>
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.blindbox.enabledItems') }}</p>
        </div>
        <div class="card p-4 text-center">
          <p class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ stats.total_draws }}</p>
      ...
      <div class="card overflow-hidden">
        ...
        <div v-else-if="items.length === 0" class="py-12 text-center text-sm text-gray-400 dark:text-gray-500">
          {{ t('admin.blindbox.empty') }}
        </div>
        <table v-else class="w-full text-sm">
          <thead class="border-b border-gray-100 bg-gray-50 dark:border-dark-700 dark:bg-dark-800">
            <tr>
              <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">{{ t('admin.blindbox.colName') }}</th>
              <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">{{ t('admin.blindbox.colRarity') }}</th>
              <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">{{ t('admin.blindbox.colRewardType') }}</th>
              <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">{{ t('admin.blindbox.colReward') }}</th>
              <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">{{ t('admin.blindbox.colWeight') }}</th>
              <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">{{ t('admin.blindbox.colStatus') }}</th>
              <th class="px-4 py-3 text-right font-medium text-gray-500 dark:text-gray-400">{{ t('admin.blindbox.colActions') }}</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-50 dark:divide-dark-800/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-dark-800/50">
              ...
              <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ rewardTypeLabel(item.reward_type) }}</td>
              <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ formatReward(item) }}</td>
              <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ item.weight }}</td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3 text-right">
                <button @click="editItem(item)" class="mr-2 text-primary-600 hover:text-primary-700 dark:text-primary-400">{{ t('common.edit') }}</button>
                <button @click="deleteItem(item)" class="text-red-500 hover:text-red-600">{{ t('common.delete') }}</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div v-if="showCreate || editingItem" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="closeModal">
        <div class="card mx-4 w-full max-w-lg space-y-4 p-6">
          ...
          <div class="space-y-3">
            ...
            <div v-if="form.reward_type === 'subscription'" class="grid grid-cols-2 gap-3">
              <div>
...
function formatReward(item: PrizeItem): string {
  switch (item.reward_type) {
    ...
    case 'subscription': return `${item.subscription_days}天`
