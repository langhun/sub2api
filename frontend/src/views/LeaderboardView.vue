<template>
  <div class="relative flex min-h-screen flex-col bg-gray-50 dark:bg-dark-950">
	<header class="border-b border-gray-200/70 px-4 py-4 dark:border-dark-800">
	  <nav class="mx-auto flex max-w-7xl items-center justify-between">
		...
		<router-link to="/login" class="text-sm text-primary-600 hover:underline dark:text-primary-400">{{ t('auth.login') }}</router-link>
	  </nav>
	</header>
	...
	<footer class="border-t border-gray-200/70 px-4 py-6 text-center text-sm text-gray-500 dark:border-dark-800 dark:text-dark-400">
...
const tabs = computed(() => {
  ...
  return [
    { key: 'balance' as TabKey, label: t('leaderboard.tabs.balance'), enabled: settings?.leaderboard_balance_enabled !== false },
    { key: 'consumption' as TabKey, label: t('leaderboard.tabs.consumption'), enabled: settings?.leaderboard_consumption_enabled !== false },
    { key: 'checkin' as TabKey, label: t('leaderboard.tabs.checkin'), enabled: settings?.leaderboard_checkin_enabled !== false },
...
onMounted(() => {
  const load = async () => {
    ...
    if (first && !tabs.value.some((tab) => tab.key === activeTab.value)) activeTab.value = first.key
