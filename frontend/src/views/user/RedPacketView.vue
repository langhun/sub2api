  ...
  <AppLayout>
    <div class="mx-auto max-w-2xl space-y-5">
      <section class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#ff355c] via-[#ff5b46] to-[#ff8c30] px-6 py-7 shadow-lg">
        ...
        <div
          data-testid="redpacket-hero-body"
          class="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-5"
        >
          ...
          <div
            data-testid="redpacket-hero-balance"
            class="w-full text-left sm:w-auto sm:text-right"
          >
            <p class="text-xs text-white/70">{{ t('redpacket.currentBalance') }}</p>
            <p class="mt-1 break-all text-2xl font-bold leading-tight text-white sm:text-[1.75rem]">
          ...
          <template v-else-if="myPackets.length > 0">
            <div class="space-y-3">
              <article v-for="rp in myPackets" :key="rp.id" class="overflow-hidden rounded-xl border border-gray-100 bg-white transition-all hover:border-gray-200 dark:border-dark-700 dark:bg-dark-800 dark:hover:border-dark-600">
                <div class="flex flex-col gap-3 px-4 py-3.5 sm:flex-row sm:items-center sm:gap-3.5">
                  ...
                  <div class="flex items-start justify-between gap-2 sm:items-center">
                    <div class="min-w-0 flex-1 text-left sm:flex-none sm:text-right">
                      <p
                        data-testid="redpacket-packet-amount"
                        class="break-all text-lg font-bold leading-tight text-rose-600 dark:text-rose-400"
                      >
                      ...
                      <div class="space-y-1.5">
                        <div v-for="claim in detailClaims" :key="claim.id" class="flex items-center justify-between rounded-lg bg-white px-3 py-2.5 shadow-sm dark:bg-dark-800">
                          <div class="flex min-w-0 items-center gap-2.5">
                            ...
                            <div class="min-w-0">
                              <p class="truncate text-sm font-medium text-slate-700 dark:text-dark-200">
                                {{ claim.user_display_name?.trim() || claim.user_email || '#' + claim.user_id }}
                              </p>
                          ...
                          <div class="ml-3 flex flex-col items-end gap-1">
                            <span v-if="isBestLuckClaim(rp, claim)" class="inline-flex items-center rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-semibold text-amber-600 dark:bg-amber-900/20 dark:text-amber-300">
...
function isBestLuckClaim(rp: RedPacketRecord, claim: RedPacketClaimRecord) {
  if (rp.redpacket_type !== 'random') return false
  if (detailClaims.value.length <= 1 || detailClaims.value.length < rp.total_count) return false
  const maxAmount = Math.max(...detailClaims.value.map((item) => item.amount))
  return Math.abs(claim.amount - maxAmount) < 1e-8
}

async function copyCode(code: string) {
  const success = await copyToClipboard(code, t('redpacket.copySuccess'))
  if (!success) return
  copiedCode.value = code
  setTimeout(() => { copiedCode.value = '' }, 2000)
}

function redPacketErrorMessage(err: unknown, fallbackKey: 'redpacket.createFailed' | 'redpacket.claimFailed') {
  return extractI18nErrorMessage(err, t, 'redpacket.errors', t(fallbackKey))
}
...
async function handleCreate() {
  ...
  try {
    ...
    createError.value = redPacketErrorMessage(e, 'redpacket.createFailed')
  } finally {
...
async function handleClaim() {
  ...
  try {
    ...
    claimError.value = redPacketErrorMessage(e, 'redpacket.claimFailed')
  } finally {
