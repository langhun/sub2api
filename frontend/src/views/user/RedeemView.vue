...
const isBalanceType = (type: string) => {
  return type === 'balance' || type === 'admin_balance' || type === 'checkin' || type === 'checkin_luck' || type === 'checkin_blindbox' || type === 'registration' || type === 'invitation'
}
...
const isBlindboxConcurrency = (item: RedeemHistoryItem) => item.type === 'checkin_blindbox' && (item.notes?.includes('Concurrency') || item.notes?.includes('并发'))

const isBlindboxSubscription = (item: RedeemHistoryItem) => item.type === 'checkin_blindbox' && (item.notes?.includes('Subscription') || item.notes?.includes('订阅'))

const isBlindboxInvitationCode = (item: RedeemHistoryItem) => item.type === 'checkin_blindbox' && (item.notes?.includes('Invitation Code') || item.notes?.includes('邀请码'))

const getBlindboxRarity = (item: RedeemHistoryItem): string | null => {
...
const getBlindboxRarityStyle = (item: RedeemHistoryItem) => {
...
const getIconName = (item: RedeemHistoryItem) => {
...
const getHistoryItemTitle = (item: RedeemHistoryItem) => {
  ...
  } else if (item.type === 'checkin_blindbox') {
