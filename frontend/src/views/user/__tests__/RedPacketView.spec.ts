...
describe("RedPacketView claim detail display name", () => {
  ...
  it("falls back to email only when username is empty", async () => {
    getRedPacketDetail.mockResolvedValueOnce({
      redpacket: { id: 7 },
      claims: [
        {
          id: 3,
          redpacket_id: 7,
          user_id: 5,
          user_email: "fallback-only@example.com",
          user_display_name: "",
          amount: 8,
          transfer_id: 13,
    ...
    const text = wrapper.text();
    expect(text).toContain("fallback-only@example.com");
  });

  it("marks the highest random claim as best luck after the packet is fully claimed", async () => {
    const wrapper = mount(RedPacketView);
    ...
    expect(wrapper.text()).toContain("redpacket.bestLuck");
  });

  it("uses mobile-safe layout classes for the hero balance summary and packet amount", async () => {
    const wrapper = mount(RedPacketView);

    await flushPromises();

    const heroBody = wrapper.find('[data-testid="redpacket-hero-body"]');
    const heroBalance = wrapper.find('[data-testid="redpacket-hero-balance"]');
    const packetAmount = wrapper.find('[data-testid="redpacket-packet-amount"]');

    expect(heroBody.exists()).toBe(true);
    expect(heroBody.classes()).toContain("flex-col");
    expect(heroBody.classes()).toContain("sm:flex-row");

    expect(heroBalance.exists()).toBe(true);
    expect(heroBalance.classes()).toContain("w-full");
    expect(heroBalance.classes()).toContain("sm:w-auto");

    expect(packetAmount.exists()).toBe(true);
    expect(packetAmount.classes()).toContain("break-all");
  });
});
