<template>
  <div class="flex min-h-screen items-center justify-center bg-[var(--background)] p-4">
    <div
      class="w-full max-w-md space-y-4 rounded-xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm"
    >
      <!-- Amount + Order ID -->
      <div v-if="amount" class="text-center">
        <p class="text-3xl font-bold text-[var(--foreground)]">&yen;{{ amount }}</p>
      ...
      <div v-if="error" class="space-y-3">
        <div
          class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3 text-sm text-red-600 dark:text-red-400"
        >
          {{ error }}
        </div>
        <button
          class="w-full text-sm text-[var(--foreground)] underline dark:text-gray-400 dark:hover:text-gray-300"
          @click="closeWindow"
        >
          {{ t('common.close') }}
        </button>
      ...
      <div v-else-if="success" class="space-y-3 py-4 text-center">
        <div class="text-5xl text-[var(--foreground)]">&#10003;</div>
        <p class="text-sm text-gray-500 dark:text-slate-400">{{ t('payment.result.success') }}</p>
        <button
          class="text-sm text-[var(--foreground)] underline dark:text-gray-400 dark:hover:text-gray-300"
          @click="closeWindow"
        >
          {{ t('common.close') }}
        </button>
