    ...
    <div class="mx-auto max-w-5xl space-y-6">
      ...
      <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div class="card p-4">
          <div class="flex items-center gap-3">
            <div class="feature-icon feature-icon-success rounded-xl p-2.5">
              <svg class="h-5 w-5 text-current" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            ...
            <div>
              ...
              <p class="text-lg font-bold text-gray-900 dark:text-white" :title="checkinBalanceDisplay.full">{{ checkinBalanceDisplay.display }}</p>
            </div>
          </div>
        </div>
        <div class="card p-4">
          <div class="flex items-center gap-3">
            <div class="feature-icon feature-icon-warning rounded-xl p-2.5">
              <svg class="h-5 w-5 text-current" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        ...
        <div class="card p-4">
          <div class="flex items-center gap-3">
            <div class="feature-icon feature-icon-info rounded-xl p-2.5">
              <svg class="h-5 w-5 text-current" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        ...
        <div class="card p-4">
          <div class="flex items-center gap-3">
            <div class="feature-icon feature-icon-purple rounded-xl p-2.5">
              <svg class="h-5 w-5 text-current" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        ...
        <div class="relative p-6">
          <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-start gap-4">
              <div class="feature-icon feature-icon-warning rounded-lg p-3.5 shadow-sm">
                <svg class="h-7 w-7 text-current" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
              ...
              <div>
                ...
                <div class="mt-1 space-y-1">
                  <template v-if="checkinStore.canCheckin">
                    ...
                    <p v-if="checkinStore.luckEnabled" class="text-sm text-gray-600 dark:text-gray-400">
            ...
            <div class="flex items-center gap-3 sm:flex-col sm:items-stretch">
              ...
              <div class="flex gap-2">
                ...
                <div v-else class="feature-pill feature-pill-success flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium">
                  <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  {{ t('checkin.checked') }}
                </div>
              </div>
      ...
      <div v-if="calendarDays.length > 0" class="card p-6">
        ...
        <div class="grid grid-cols-7 gap-1.5 sm:gap-2">
          ...
          >
            ...
            <span v-if="day.checkedIn" class="mt-0.5 text-[10px] font-semibold leading-none" :class="day.rewardType === 'luck' ? 'text-gray-600 dark:text-gray-400' : 'text-gray-600 dark:text-gray-400'">
        ...
        <div class="mt-3 flex items-center justify-center gap-5 text-xs text-gray-400 dark:text-dark-500">
          <div class="feature-pill feature-pill-success flex items-center gap-1.5 px-2.5 py-1"><span class="text-[10px] font-semibold">+$0.00</span>{{ t('checkin.normalCheckin') }}</div>
          <div class="feature-pill feature-pill-warning flex items-center gap-1.5 px-2.5 py-1"><span class="text-[10px] font-semibold">+$0.00</span>{{ t('checkin.luckCheckin') }}</div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        ...
        <div class="card p-5">
          ...
          <div v-if="checkinStore.todayReward !== null && !checkinStore.canCheckin" class="space-y-3">
            <div class="flex items-center justify-between">
              ...
              <span class="text-sm font-bold" :class="checkinStore.todayReward >= 0 ? 'text-gray-900 dark:text-white' : 'text-red-500 dark:text-red-400'">
            ...
            <div class="flex items-center justify-between">
              ...
              <span class="feature-pill rounded-full px-2.5 py-0.5 text-xs font-medium" :class="checkinStore.todayCheckinType === 'luck' ? 'feature-pill-warning' : 'feature-pill-success'">
            ...
            <div v-if="checkinStore.todayCheckinType === 'luck' && checkinStore.todayMultiplier" class="flex items-center justify-between">
              ...
              <span class="text-sm font-semibold text-gray-900 dark:text-white">x{{ checkinStore.todayMultiplier?.toFixed(2) }}</span>
            </div>
            <div v-if="checkinStore.blindboxResult" class="feature-panel-purple mt-2 rounded-lg border p-3">
              <div class="mb-1.5 flex items-center justify-between">
                <span class="text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('checkin.page.todayBlindbox') }}</span>
              ...
              <div v-if="checkinStore.blindboxResult.reward_type === 'invitation_code' && checkinStore.blindboxResult.reward_detail" class="feature-pill feature-pill-info mt-1.5 rounded px-2 py-1 text-xs font-mono">
        ...
        <div class="card p-5">
          ...
          <div v-if="checkinStore.status?.blindbox_enabled" class="space-y-2 text-xs text-gray-600 dark:text-gray-400">
            ...
            <div v-if="nextBlindboxHint" class="feature-panel-info mt-2 rounded-lg border p-2.5 text-center">
              <span class="text-gray-700 dark:text-gray-300">{{ nextBlindboxHint }}</span>
            </div>
          </div>
          <div v-else class="py-3 text-center">
            <p class="text-xs text-gray-400 dark:text-dark-500">{{ t('checkin.page.noBlindbox') }}</p>
      ...
      <div class="card p-6">
          ...
          <div v-for="record in blindboxRecords" :key="record.id" class="flex items-center justify-between rounded-lg border px-4 py-3 blindbox-record" :class="getRecordBorderClass(record.rarity)">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <span v-if="record.reward_type === 'invitation_code' && record.reward_detail" class="ml-2 rounded bg-gray-50 px-2 py-0.5 text-xs font-mono text-gray-600 dark:bg-gray-900/30 dark:text-gray-400">{{ record.reward_detail }}</span>
              </div>
            </div>
        ...
        <div v-if="blindboxTotal > blindboxRecords.length" class="mt-4 text-center">
          <button type="button" class="text-sm text-[var(--foreground)] hover:underline" @click="loadMoreBlindboxRecords">{{ t('common.loadMore') }}</button>
        </div>
      </div>
    </div>
  </AppLayout>
...
import { formatDualDisplayAmount, formatSignedCurrency } from '@/utils/format'
...
const checkinBalanceDisplay = computed(() =>
...
const streakColor = computed(() => {
  ...
  if (d >= 30) return 'text-gray-900 dark:text-white'
  if (d >= 7) return 'text-gray-700 dark:text-gray-200'
...
function getRecordBorderClass(rarity: string) {
  const map: Record<string, string> = {
    common: 'border-slate-200 dark:border-dark-700',
    rare: 'border-gray-300 dark:border-dark-600',
    epic: 'border-gray-400 dark:border-dark-500',
    legendary: 'border-gray-500 dark:border-gray-500/70',
  }
...
function getRecordRewardColor(record: { rarity: string }) {
  const map: Record<string, string> = {
    ...
    rare: 'text-gray-700 dark:text-gray-200',
    epic: 'text-gray-800 dark:text-gray-100',
    legendary: 'text-gray-900 dark:text-white',
  }
...
function formatRecordReward(record: BlindboxRecordItem) {
  switch (record.reward_type) {
    case 'balance': return `+${formatDualDisplayAmount(record.reward_value, { currencySymbol: '$' }).display}`
...
function formatBlindboxReward(result: BlindboxResult) {
  switch (result.reward_type) {
    case 'balance': return `+${formatDualDisplayAmount(result.reward_value, { currencySymbol: '$' }).display}`
...
const rarityBreakdown = computed(() => {
  ...
  const barClasses: Record<string, string> = {
    common: 'bg-slate-300 dark:bg-slate-600',
    rare: 'bg-slate-400 dark:bg-slate-500',
    epic: 'bg-slate-500 dark:bg-slate-400',
    legendary: 'bg-slate-700 dark:bg-slate-300',
  }
...
.badge-common { background-color: var(--muted); color: var(--muted-foreground); }
.badge-rare { background-color: var(--card); color: var(--foreground); }
.badge-epic { background-color: color-mix(in oklch, var(--foreground) 6%, var(--card)); color: var(--foreground); }
.badge-legendary { background-color: color-mix(in oklch, var(--foreground) 12%, var(--card)); color: var(--foreground); }
html.dark .badge-common { background-color: var(--muted); color: var(--muted-foreground); }
html.dark .badge-rare { background-color: var(--card); color: var(--foreground); }
html.dark .badge-epic { background-color: color-mix(in oklch, var(--foreground) 8%, var(--card)); color: var(--foreground); }
html.dark .badge-legendary { background-color: color-mix(in oklch, var(--foreground) 14%, var(--card)); color: var(--foreground); }
...
.checkin-btn-normal { background: var(--foreground); }
.checkin-btn-normal:hover:not(:disabled) { background: var(--foreground); opacity: 0.9; }
.checkin-btn-luck { background: var(--foreground); }
.checkin-btn-luck:hover:not(:disabled) { background: var(--foreground); opacity: 0.9; }

.calendar-cell { min-height: 40px; }
.calendar-cell-checked { background-color: var(--muted); border: 1px solid var(--border); }
.calendar-cell-luck { background-color: var(--muted); border: 1px solid var(--border); }
.calendar-cell-missed { background-color: transparent; border: 1px solid var(--border); }
.calendar-cell-today { box-shadow: 0 0 0 2px var(--ring); z-index: 1; }
html.dark .calendar-cell-checked { background-color: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.2); }
html.dark .calendar-cell-luck { background-color: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.2); }
html.dark .calendar-cell-missed { border-color: var(--border); }
html.dark .calendar-cell-today { box-shadow: 0 0 0 2px var(--ring); }
</style>
