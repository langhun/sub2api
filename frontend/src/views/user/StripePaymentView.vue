  ...
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-gray-50 dark:bg-dark-900' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        <div class="feature-icon feature-icon-danger mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-xl">
          <Icon name="exclamationCircle" size="xl" class="text-current" />
        </div>
        ...
        <button class="btn btn-secondary mt-6" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
      </div>
      <template v-else>
        <!-- 金额头部 -->
        <div v-if="order" class="card feature-panel-info overflow-hidden">
          <div class="px-6 py-6 text-center">
            <p class="text-sm font-medium text-[var(--muted-foreground)]">{{ t('payment.actualPay') }}</p>
            <p class="mt-1 text-3xl font-bold text-[var(--foreground)]">{{ formatGatewayAmount(order.pay_amount) }}</p>
          </div>
        </div>
        ...
        <template v-if="wechatQrUrl">
          <div class="card feature-panel-info p-6">
            <div class="flex flex-col items-center space-y-4">
              ...
              <div class="relative rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
                <img :src="wechatQrUrl" alt="WeChat Pay QR" class="h-56 w-56 rounded" />
                <div class="pointer-events-none absolute inset-0 flex items-center justify-center">
                  <span class="rounded-lg border border-[var(--border)] bg-[var(--muted)] p-2 text-[var(--foreground)]">
        ...
        <template v-else-if="redirecting">
          <div class="card feature-panel-info p-6">
            <div class="flex flex-col items-center space-y-4 py-4">
              <div class="h-10 w-10 animate-spin rounded-full border-4 border-gray-300 border-t-transparent dark:border-dark-600"></div>
