        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div class="card p-5">
            ...
            <p class="mt-2 text-2xl font-semibold text-emerald-600 dark:text-emerald-400">
              {{ formatAffiliateCurrency(detail.aff_quota) }}
            </p>
          </div>
          <div class="card p-5">
            <p class="text-sm text-gray-500 dark:text-dark-400">{{ t('affiliate.stats.totalQuota') }}</p>
            <p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">
              {{ formatAffiliateCurrency(detail.aff_history_quota) }}
            </p>
            <p v-if="detail.aff_frozen_quota > 0" class="mt-1 text-xs text-amber-600 dark:text-amber-400">
              {{ t('affiliate.stats.frozenQuota') }}: {{ formatAffiliateCurrency(detail.aff_frozen_quota) }}
            </p>
          </div>
        </div>
            ...
            <table class="w-full min-w-[560px] text-left text-sm">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-3 py-3 text-right font-medium text-emerald-600 dark:text-emerald-400">{{ formatAffiliateCurrency(item.total_rebate) }}</td>
...
function formatAffiliateCurrency(value: number | null | undefined): string {
...
async function transferQuota(): Promise<void> {
  ...
  try {
    ...
    appStore.showSuccess(t('affiliate.transfer.success', { amount: formatAffiliateCurrency(resp.transferred_quota) }))
    await Promise.all([
