export default {
  ...
  transfer: {
    title: '余额转账', description: '核对接收方与费用后完成站内余额转账', totalSent: '累计转出', totalReceived: '累计转入', totalFee: '累计手续费', newTransfer: '发起转账', irreversible: '转账到账后不可由用户撤销，请仔细核对接收方。', receiverId: '接收方用户 ID', recipient: '接收方', amount: '到账金额', feePreview: '手续费', total: '合计扣款', memo: '留言（可选）', continue: '核验并继续', success: '转账成功', successWithId: '转账成功，流水号 #{id}', submit: '确认转账', failed: '转账失败，请稍后重试', validationFailed: '无法核验接收方或费用，请检查输入', confirmTitle: '确认余额转账', confirmMessage: '请再次核对本次转账，确认后将立即到账。', confirmRecipient: '接收方：{recipient}', confirmAmount: '到账 {amount}，手续费 {fee}，合计扣款 {total}', history: '转账明细', historyHint: '按转出与转入查看已完成的资金记录', historyFailed: '转账记录加载失败', emptyHistory: '暂无转账记录', sent: '我转出的', received: '我收到的', toUser: '转给用户 #{id}', fromUser: '来自用户 #{id}', status: { completed: '已完成', frozen: '已标记异常', revoked: '已撤销' }
  },
  redpacket: {
    title: '红包中心', description: '创建并分享红包口令，或领取好友发送的红包', create: '发红包', claim: '领红包', myPackets: '我的红包', sent: '我发出的', received: '我领取的', sentHistoryHint: '查看红包剩余份数、金额和有效期', totalAmount: '总金额', count: '红包份数', type: '分配方式', equal: '等额红包', equalHint: '每份金额相同', random: '拼手气红包', randomHint: '由服务端随机分配每份金额', memo: '祝福语', memoPlaceholder: '恭喜发财', perPacket: '预计每份', confirmTitle: '确认发红包', confirmMessage: '将发出 {count} 份、共 {amount} 的红包，确认继续？', createdTitle: '红包已创建', shareHint: '分享以下红包口令，对方即可领取。', createFailed: '红包创建失败，请检查金额和份数', claimTitle: '输入红包口令', claimHint: '口令会自动去除首尾空格并统一为大写', code: '红包口令', codePlaceholder: '输入红包口令', claimSuccess: '领取成功', claimFailed: '领取失败，请核对口令或红包状态', historyFailed: '红包记录加载失败', detailFailed: '红包详情加载失败', claimRecords: '领取记录', empty: '暂无红包记录', remaining: '剩余 {remaining}/{total} 份', detailTitle: '红包详情', remainingAmount: '剩余金额', expiresAt: '过期时间', status: { active: '领取中', expired: '已过期', exhausted: '已领完' }
  }
  ,gameHall: {
  ...
  activityErrors: { featureDisabled: '该功能当前未开放', insufficientBalance: '可用余额不足', limitExceeded: '已超过当前额度或次数限制', alreadyClaimed: '您已经领取过该红包', redpacketExpired: '红包已过期', redpacketExhausted: '红包已领完', redpacketNotFound: '红包口令不存在', cannotClaimOwn: '不能领取自己发出的红包', duplicateRequest: '请求正在处理或已提交，请勿重复操作' }
}
